...
- **Spec → Plan → Plan-review → Build → Review.** A task that is unclear, spans more than one module, or changes behavior at a seam runs through the gates before any code. `/spec` ([`.claude/skills/spec/`](.claude/skills/spec/SKILL.md)) maps *what & why* — Why (5) → Scope (5) → code-grounded interrogation (read first, cite `path:line`) → a GitHub issue. `/plan` ([`.claude/skills/plan/`](.claude/skills/plan/SKILL.md)) maps *how* — the drafter runs a scope challenge (≈8-file / 2-module smell stops for a cut) and an engineering lens (ASCII data-flow diagram + edge cases + failing-test-first plan), then an **independent reviewer sub-agent** (not the drafter, not the implementer — Software Architect by default; product/design lenses scale in) adversarially reviews the plan and returns `ready` / `revise` + unresolved decisions. The decision and verdict are recorded on the issue. Build delegates to the owning roster sub-agent (TDD); the Code Reviewer reviews the diff. A typo / one-line fix skips the gates.
...
| Spec → Plan → independent plan-review before build on non-trivial work | Working style (`/spec`, `/plan` skills) |
