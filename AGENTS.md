...
- **Spec → Plan → Build → Review.** A task that is unclear, spans more than one module, or changes behavior at a seam runs through the spec and plan gates before any code. `/spec` ([`.claude/skills/spec/`](.claude/skills/spec/SKILL.md)) maps *what & why* — Why (5) → Scope (5) → code-grounded interrogation (read first, cite `path:line`) → a GitHub issue. `/plan` ([`.claude/skills/plan/`](.claude/skills/plan/SKILL.md)) maps *how* — scope challenge (≈8-file / 2-module smell stops for a cut), an engineering lens with an ASCII data-flow diagram + edge cases + a failing-test-first plan, and the design decision recorded in `docs/`. Build and review follow with the roster sub-agents. A typo / one-line fix skips the gates.
...
| Spec → Plan gates before build on non-trivial work | Working style (`/spec`, `/plan` skills) |
