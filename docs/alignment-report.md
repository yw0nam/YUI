...
| V3 | Control envelope transport | ✅ **확정: 서버사이드 `express` tool-call.** Hermes `/v1/responses`가 `function_call` 아이템 노출 + `GET /v1/runs/{id}/events` SSE → `express(...)` skill arguments(`{emotion, motion, should_speak}`)로 제어신호 전송. 발화는 별도 텍스트 스트림 | **transport 확정**(D-TRANSPORT/D-SPEECH, contract §Endpoint/§3). json_schema 강제는 이론적 fallback으로 강등. 잔여 미검증은 OI-2로 이동 | Hermes docs `/features/api-server` (function_call 노출 검증) + 사용자 결정(2026-06) |
...
- **[중대] control transport 확정:** json_schema strict-output 강제 가정을 **서버사이드 `express` tool-call**(arguments `{emotion, motion, should_speak}`)로 supersede (contract §Endpoint/§3, PRD F6/F9/§5 D-TRANSPORT, dispatcher A3). 발화 텍스트는 별도 assistant 텍스트 스트림으로 분리(D-SPEECH).
...
- emotion/motion/envelope 필드명: 3문서 일치 확인 (motion `idle/drag/sit`, envelope `emotion/motion/should_speak/speech_text/tool_status/rich_content/_reserved`).
