...
- **Source**: all clips are extracted from the [Mate Engine](https://github.com/shinyflvre/Mate-Engine) Unity project by Shiny (`Assets/MATE ENGINE - Animations/…`) to `.vrma` via the `unity-cli exec → YuiExport.YuiVrmaExport.ExportBatch` pipeline (30 FPS, T-pose reference, model-independent retarget). Durations are the source clip length. Used under Mate Engine's non-commercial terms — attribution to Shiny required; commercial use needs separate permission.
...
- **Descriptive exception**: motions whose action or pose is itself the clearest identifier keep that descriptive name instead of an emotion label — `idle`, `drag`, `peek`, `sit`, `window_sit`, `sleeping`, `dance`.
