...
| `window_sit` | state | yes | Held window-perch (see below). Plays a sit variant, `pingpong: true` with `loop_cycles: [1, 3]` (forward↔reverse a random 1–3 round trips), holds the settled frame for `cycle_dwell_ms`, crossfades to a different variant, repeats until interrupted. `broker_publish: false`. Engaged by a dev trigger. | PET_SITTING/* | — |
...
Both pools share the same 8 variants — a mix of floor sits and edge/perch sits. All lower the hips when seated. `sit` is a oneshot that returns to idle; `window_sit` is a looping held state that ping-pongs each variant forward↔reverse a random 1–3 round trips (`loop_cycles: [1, 3]`), holds the settled frame for `cycle_dwell_ms`, then crossfades to a different variant.
