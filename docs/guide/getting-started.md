...
`generate_express` tool-call deltas are parsed from the `chat.completion.chunk` stream as they arrive and the cue plays at that moment. Because a tool-calling turn usually carries no speech, the client returns the tool result and re-requests so the model continues into speech — up to three round trips per turn. A turn with no tool call is a single request. See [CC mode transport](../reference/backend-contract.md#cc-mode-transport-chat-completions) for the wire shape.
