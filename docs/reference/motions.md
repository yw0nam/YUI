...
| `climb_up` | reactive | yes | Wall-climb loop held for the length of an ascent. The VRM stays at scene origin while the OS window rises along the chosen window edge and the root yaws ±90° toward the wall; the window is paced at 0.93 m per cycle, divided by the loaded clip's own duration, so the hands do not slide. `broker_publish: false`. | Mixamo "Climbing UP" | 2.97s |
| `climb_up_done` | oneshot | no | Pull-over onto the ledge that ends an ascent; the window rises the clip's own 1.38 m linearly over its length, leaving the feet on the window's top edge. `broker_publish: false`. | Mixamo "Climbing_UP_done" | 3.8s |
| `climb_down` | reactive | yes | Wall-descent loop held for the length of a descent, paced at 1.15 m per cycle. It also covers the step off the ledge: the clip crossfades in over a 400 ms linear slide down the wall. `broker_publish: false`. | Mixamo "Climbing Down Wall" | 2.0s |
| `climb_down_landing` | oneshot | no | Touchdown at the end of a descent; the window drops the clip's own 0.25 m linearly over its length onto the work-area floor. `broker_publish: false`. | Mixamo "Climbing Down_landing" | 1.38s |
