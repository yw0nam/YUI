...
- **Triggers**: AI via `generate_express` `motion_id` plays any broker-published oneshot/pool; `idle` is the ambient baseline; `drag` is the reactive pickup; `head_pat` loops while a press is held on the head region; `window_sit` is a held state engaged by a dev trigger; `thinking` is a client-played TTFT latency affordance (loops from the start of a backend turn until the response speech begins); `walk` is the ambient floor or foreign-window-top stroll, played by the client's walk schedulers while the OS window translates along the supporting edge; `falling` and `landing` are the fall chain, played by the client when a drag release leaves the character mid-air or a perched sit loses its host window, and the window drops to the work-area floor; `climb_up`, `climb_up_done`, `climb_down` and `climb_down_landing` are the climb chain, played by the client's climb scheduler while the OS window travels up or down a foreign window's side edge.
