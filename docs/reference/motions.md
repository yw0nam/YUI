...
| `walk` | state | yes | In-place walk cycle held for the length of an ambient stroll — the VRM stays at scene origin while the OS window translates along the work-area bottom, and the root yaws ±90° toward the travel direction. Priority 55 (above `idle` and `thinking`, below reactive emotes, which end the stroll by taking the motion). `broker_publish: false`. | Mixamo Walking | 1.3s |
