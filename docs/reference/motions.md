...
| `stand_up` | oneshot | no | Standing up from a window seat, played before a perched stroll or a climb descent leaves the seat. `root_lock_y: true`: the pin is released and the window follows the clip's rise, scaled to the same seat distance, so the feet end on the edge. `broker_publish: false`. | Mixamo "Sit To Stand" | 2.0s |
