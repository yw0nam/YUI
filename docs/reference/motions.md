...
| `peek` | state | yes (LoopRepeat; wrap is hidden behind the occluding window) | Held while side-perched; the hips stay pinned inside the app-window edge, and the clip mirrors on its configured side while walking out of view. `broker_publish: false`. | PET_HIDING/PET_HIDE | 14.0s |
