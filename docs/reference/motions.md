...
- **Sources**: clips come from three origins.
  ...
  - **[Mixamo](https://www.mixamo.com/)** — `walk` ("Walking", in-place), `falling` ("Falling") and `landing` ("Falling To Landing"). Royalty-free under the Mixamo terms; converted to `.vrma` through the Mate-Engine `YuiExport.YuiVrmaExport` pipeline (30 FPS) with a with-skin (bind-pose) source avatar; the fall pair is exported from the "Maria WProp J J Ong" character.
