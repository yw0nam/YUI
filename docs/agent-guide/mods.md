...
4. Add `Mods/<name>` to the repository-relative path list in BOTH CI loops (`mods` and `mods-lint` in `.github/workflows/ci.yml`) — the jobs iterate a hardcoded list, not the filesystem.
...
Ruff is the only Python linter (`format --check` + `check`, `line-length = 110`) for the Python projects covered by those loops.
