...
- **client-side TTS 파이프라인(확정, D-TTS-PIPELINE):** 텍스트 스트림 토큰을 **버퍼 큐에 적재** → **문장 분절(sentence boundary) 감지** → 분절된 부분까지를 **per-sentence로 TTS API 호출** → output wav → UI 재생. (TTS는 `tts_provider`로 swap — default irodori `/synthesize` @8091[reference-voice 기반], openai-compatible `/audio/speech` @8092. contract.md §5 참고.) **재생 순서 보존**(TTS 응답이 순서 뒤바뀌어 와도 원래 문장 순서대로 재생).
...
| D-LIPSYNC-GAIN | **립싱크 입 벌림 게인 = 런타임 조절 (빠른 설정 슬라이더).** 진폭→입 게인(D1)을 "입 움직임" 슬라이더로 노출(0.5×–4.0×, 기본 2.0×, localStorage persist, key `yui.lipsync`). 드래그 중 `renderer.setMouthOpen(clamp(gain*0.15,0,1))`으로 실제 VRM 입을 라이브 프리뷰하고 release/close 시 `stopMouth`. 값은 `createWebAudioSink({getGain})`로 TTS 재생 envelope에 주입돼 다음 발화 clip부터 반영. 범위/기본값 상수는 `src/io/lipsync-settings.ts`의 UI bound이며 endpoints류 런타임 config 아님. | feat/lipsync-gain-ui |
| D-REASONING-EFFORT | **`reasoning.effort` = per-user 런타임 설정 (config 필드 아님).** Responses 표준 파라미터 `reasoning: { effort }`를 UI에 `default · low · medium · high` 4종으로 노출한다. **`default`는 파라미터를 통째로 생략**(backend가 결정)하는 안전 기본값이고, `low\|medium\|high`만 `reasoning.effort`로 실어 보낸다. backend("natsume", OpenAI 호환)가 무시할 수 있는 best-effort hint — out-of-the-box 값은 `default`(생략). wire envelope(contract §3) 불변; 표준 요청 필드만 채운다. (contract.md §Runtime request-shaping inputs) | 2026-06-07 |
...
2. **서버사이드 `express` tool 등록 (optional 비언어 제어 채널)** — emotion/motion 연출을 원하면 OpenAI-compatible backend가 `generate_express(...)` tool-call을 스트림에 노출한다([`backend_agent_broker_interaction.md`](./backend_agent_broker_interaction.md)). client는 응답 스트림에서 `function_call`(name==`generate_express`)을 파싱(근거: `openai_response_sdk/sse-event-format.md`). **express는 optional** — 호출 안 하는 턴은 client가 idle로 처리하므로 매 턴 보장·도착 타이밍은 **하드 의존이 아니다**(R16/R17 해소). **검증(2026-06): Hermes `/v1/responses`가 function_call item을 스트림에 노출(Hermes 자체 SSE 구현 docs).** json_schema 강제는 dependency 아님.
