  ...
  output: [
    {
      ...
      source: "gmail",
      priority: "digest",
      payload: '{"source":"gmail","kind":"email","id":"18f123"}',
      status: "pending",
  ...
  config: {
    ...
    parameters: {
      conditions: {
        ...
        conditions: [
          {
            leftValue: expr("{{ $input.all().filter(i => i.json.id != null).length }}"),
            rightValue: 0,
...
const composeDigestBody = node({
  ...
  config: {
    ...
    parameters: {
      ...
      language: "javaScript",
      jsCode:
        "const rows = $('Fetch Pending Signal Queue Rows').all().map(i => i.json).filter(r => r.id != null);\n" +
  ...
  output: [
    {
      signals: [
        {
          _directive: "workflow_reaction_guide",
          workflow: "sg-dispatch",
          how_to_react:
            "Batched digest of signals queued since the last flush (30 min cadence). Summarize concisely; only nudge the user about items that look actionable.",
        },
        { source: "gmail", kind: "email", id: "18f123" },
      ],
    },
  ],
});
...
const restorePostedRows = node({
  ...
  config: {
    ...
    parameters: {
      ...
      language: "javaScript",
      jsCode:
        "return $('Fetch Pending Signal Queue Rows').all().filter(i => i.json.id != null).map(i => ({ json: i.json }));",
    },
    position: [1300, 160],
  },
...
const markRowSent = node({
  ...
  config: {
    ...
    parameters: {
      ...
      matchType: "allConditions",
      filters: {
        conditions: [{ keyName: "id", condition: "eq", keyValue: expr("{{ String($json.id) }}") }],
      },
      columns: {
        mappingMode: "defineBelow",
        value: { status: "sent", sent_at: expr("{{ $now.toISO() }}") },
      },
      options: {},
    },
    position: [1560, 160],
...
const deleteExpiredSentRows = node({
  ...
  config: {
    ...
    parameters: {
      ...
      filters: {
        conditions: [
          { keyName: "status", condition: "eq", keyValue: "sent" },
          {
            keyName: "createdAt",
            condition: "lt",
            keyValue: expr("{{ $now.minus({ days: 14 }).toISO() }}"),
          },
        ],
      },
    },
...
const dispatchNote = sticky(
  "## Dispatch throttle contract\nFlushes all pending rows as one digest every 30 minutes. An empty queue produces silence. Immediate delivery is reserved for collector fast paths, currently CI failures.",
  [fetchPendingRows, postDigestToYui, markRowSent],
  { color: 5 },
);
...
export default workflow("sg-dispatch", "sg-dispatch")
  ...
  .to(fetchPendingRows)
  .to(
    hasPendingRows.onTrue!(
      composeDigestBody.to(
        postDigestToYui.to(restorePostedRows.to(markRowSent.to(deleteExpiredSentRows))),
      ),
