...
const fetchOpenPullRequests = node({
  ...
  output: [
    {
      number: 42,
      title: "Improve signal handling",
      html_url: "https://github.com/yw0nam/YUI/pull/42",
      draft: false,
...
const buildPullRequestCandidates = node({
  ...
  config: {
    ...
    parameters: {
      ...
      language: "javaScript",
      jsCode:
        "return $input.all().filter(i => i.json.number != null && !i.json.draft).map(i => {\n" +
        "  const p = i.json;\n" +
  ...
  output: [
    {
      key: "pr:yw0nam/YUI#42",
      source: "repo-status",
      priority: "digest",
      payload: '{"source":"github_pr","repo":"yw0nam/YUI","n":42}',
    },
  ],
});
...
const fetchWorkflowRuns = node({
  ...
  config: {
    ...
    parameters: {
      method: "GET",
      url: expr(
        "https://api.github.com/repos/{{ $json.owner }}/{{ $json.name }}/actions/runs?per_page=15",
      ),
      authentication: "none",
      sendHeaders: true,
      ...
      workflow_runs: [
        {
          ...
          name: "CI",
          head_branch: "main",
          html_url: "https://github.com/yw0nam/YUI/actions/runs/9001",
          created_at: "2026-07-19T00:00:00.000Z",
          repository: { full_name: "yw0nam/YUI" },
        },
      ],
    },
  ],
...
const buildCiCandidates = node({
  ...
  config: {
    ...
    parameters: {
      ...
      language: "javaScript",
      jsCode:
        "const runs = $input.all().flatMap(i => Array.isArray(i.json.workflow_runs) ? i.json.workflow_runs : (i.json.id != null ? [i.json] : []));\n" +
  ...
  output: [
    {
      key: "ci:yw0nam/YUI#9001",
      source: "repo-status",
      priority: "immediate",
      payload: '{"source":"github_ci","repo":"yw0nam/YUI","run_id":9001,"conclusion":"failure"}',
    },
  ],
});
...
const mergeRepoCandidates = merge({
  version: 3.2,
  config: {
    name: "Merge Pull Request and CI Candidates",
...
const keepNewRepoSignals = node({
  ...
  config: {
    ...
    parameters: {
      ...
      language: "javaScript",
      jsCode:
        "const candidates = $('Merge Pull Request and CI Candidates').all().map(i => i.json).filter(i => i.key);\n" +
  ...
  output: [
    { key: "ci:yw0nam/YUI#9001", source: "repo-status", priority: "immediate", payload: "{}" },
  ],
});
...
const pushCiFailures = node({
  ...
  config: {
    ...
    parameters: {
      ...
      specifyBody: "json",
      jsonBody: expr(
        '{{ { "signals": [ { "_directive": "workflow_reaction_guide", "workflow": "sg-collect-repo-status", "how_to_react": "CI run failed. Tell the user promptly which repo/workflow failed." }, ...$("Apply PR Cooldown and CI Permanent Dedup").all().filter(i => i.json.priority === "immediate").map(i => JSON.parse(i.json.payload)) ] } }}',
      ),
      options: {},
    },
    executeOnce: true,
...
const repoStatusNote = sticky(
  "## Repository status collector\nQueues non-draft PRs and successful CI runs for the digest. Only completed CI runs created within the last 24 hours are considered. CI failures use the queue-first fast path: a sent row is inserted before the immediate gateway POST. PR keys cool down for 24 hours; CI run keys are permanently deduplicated.",
  [fetchOpenPullRequests, fetchWorkflowRuns, fetchRepoStatusHistory],
  { color: 5 },
);
...
export default workflow("sg-collect-repo-status", "sg-collect-repo-status")
  ...
  .to(
    routeImmediateCi.onTrue!(insertSentCiFailure.to(pushCiFailures)).onFalse(
      insertPendingRepoSignal,
    ),
  );
