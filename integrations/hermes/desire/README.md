...
on. Both are required and have no default; a code path that needs one and cannot find it fails with an error
naming the variable, and the desire block opens with the line `agent: <agent>` so the prompts can refer to it.
