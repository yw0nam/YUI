---
name: yui-dispatch
description: "Hand a ready-for-agent issue from any GitHub repository the user names to a headless Claude Code session that implements it and opens the pull request. Use when the user asks you to dispatch, delegate, or hand over an issue by number."
version: 0.2.0
author: yw0nam
platforms: [linux]
prerequisites:
...
metadata:
  hermes:
    tags: [yui, dispatch, claude-code]
---

# yui-dispatch

The user closes the spec inside an issue, labels it `ready-for-agent`, and tells you in plain language which one
to hand over (for example "YUI 123 맡아"). You start one headless `claude -p` session on that issue and relay the
outcome. You do not implement anything yourself; the session works from the issue and the repository's own rules,
opens the pull request, and never merges it.

`$YUI` is the absolute path of the YUI checkout. `$N` is the issue number. `$DESIRE_AGENT_NAME` and
`$HERMES_PROFILE` come from the profile environment and name the agent and the profile the pull request is opened
under. `$REPO` is the repository as `owner/name`, resolved from the name the user gives:

```bash
gh search repos <name> --match name --json fullName --jq '.[].fullName'
```

Take the entry whose name part equals what the user said. If none or more than one matches, ask which one and
stop.
...
a model the user names in the request (for example "Opus로 해줘" means `opus`), then `sonnet`. Pass the name in
lower case.
...
gh issue comment $N --repo $REPO --body "Picked up by $DESIRE_AGENT_NAME; a headless Claude Code session on $MODEL is working on it."
```
...
Reply to the user with one line: `Started #$N.` Then end the turn; the completion notification wakes you.
...
sentence, and tell the user `Dispatch could not start #$N: <reason>`.
...
- Pull request found: `gh issue comment $N --repo $REPO --body "Pull request: <url>"` and tell the user
  `Dispatched #$N → <url>`.
- No pull request (timeout, exit code other than 0, or `No pull request opened.` in the result): comment the reason on
  ...
  label, and tell the user `Dispatch failed #$N: <reason>`.

Leave the assignee in place either way. The user clears it when re-labeling the issue.

## What you do not do

- Fix review comments on the pull request. The user does that from a local Claude Code session.
- Retry a failed dispatch on your own. Wait for the issue to be labeled `ready-for-agent` again.
- Poll issues for the label. Dispatch what the user hands you, or the one self-started dispatch a day that
