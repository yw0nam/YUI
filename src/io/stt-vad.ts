...
export interface SttVadOptions {
  /** Accepts a getter so a live stt_base_url override is read on every request, not pinned at construction. */
  config: EndpointsConfig | (() => EndpointsConfig);
  fetch?: typeof fetch;
  /**
...
export function createSttVad(options: SttVadOptions): SttVad {
  ...
  async function onSpeechEnd(audio: Float32Array): Promise<void> {
    ...
    try {
      ...
      const res = await fetchImpl(`${resolveConfig().stt_base_url}/audio/transcriptions`, {
        method: "POST",
        body: form,
  ...
  return {
    start(): Promise<void> {
      ...
      if (!resolveConfig().stt_base_url) return Promise.resolve();
