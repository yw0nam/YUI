...
describe("validateMotions — top-level shape", () => {
  ...
  it("rejects an empty registry", () => {
    expectIssue({}, "at least one motion must be registered");
  });

  it("rejects a non-object entry", () => {
    expectIssue({ idle: "not-an-object" }, "entry is not an object");
  });
});
...
describe("validateMotions — field validation", () => {
  ...
  it("rejects an unknown kind", () => {
    expectIssue({ idle: baseEntry({ kind: "bogus" }) }, "kind must be");
  });

  it("rejects a non-boolean loop", () => {
    expectIssue({ idle: baseEntry({ loop: "yes" }) }, "loop must be a boolean");
  });

  it("rejects priority below 0", () => {
    expectIssue(
      { idle: baseEntry({ priority: -1 }) },
      "priority must be a finite number in [0, 100]",
    );
  });

  it("rejects priority above 100", () => {
    expectIssue(
      { idle: baseEntry({ priority: 101 }) },
      "priority must be a finite number in [0, 100]",
    );
  });
  ...
  it("rejects NaN priority", () => {
    expectIssue(
      { idle: baseEntry({ priority: Number.NaN }) },
      "priority must be a finite number in [0, 100]",
    );
  });
  ...
  it("rejects Infinity priority", () => {
    expectIssue(
      { idle: baseEntry({ priority: Number.POSITIVE_INFINITY }) },
      "priority must be a finite number in [0, 100]",
    );
  });

  it("rejects an unknown interrupt_policy", () => {
    expectIssue({ idle: baseEntry({ interrupt_policy: "bogus" }) }, "interrupt_policy must be");
  });
});
...
describe("validateMotions — variants / variant_policy", () => {
  it("rejects variants that isn't an array", () => {
    expectIssue(
      { idle: baseEntry({ variants: "not-array" }) },
  ...
  it("rejects a single-item variants array (needs >=2)", () => {
    expectIssue(
      { idle: baseEntry({ variants: ["/a.vrma"] }) },
  ...
  it("rejects a variants entry not ending in .vrma", () => {
    expectIssue(
      ...
      "variants entries must end in .vrma",
    );
  });
  ...
  it("rejects variant_policy set without variants (dead field)", () => {
    expectIssue(
      ...
      "variant_policy has no meaning without variants",
    );
  });
});
...
describe("validateMotions — broker_publish", () => {
  it("rejects a non-boolean broker_publish", () => {
    expectIssue(
      { idle: baseEntry({ broker_publish: "true" }) },
      "broker_publish must be a boolean",
    );
  });
});
...
describe("validateMotions — cycle_dwell_ms", () => {
  it("rejects a non-integer", () => {
    expectIssue(
      ...
      "cycle_dwell_ms must be an integer in [0, 60000]",
    );
  });
  ...
  it("rejects cycle_dwell_ms on a non-cycle motion (dead field)", () => {
    expectIssue(
      ...
      "cycle_dwell_ms is valid only for a cycle motion (variants>1 + loop)",
    );
  });
});
...
describe("validateMotions — pingpong / loop_cycles", () => {
  ...
  it("rejects pingpong:true without loop:true", () => {
    expectIssue(
      ...
      "pingpong:true requires loop:true",
    );
  });
  ...
  it("rejects pingpong:true combined with crossfade_loop:true", () => {
    expectIssue(
      ...
      "pingpong and crossfade_loop are mutually exclusive",
    );
  });
  ...
  it("rejects loop_cycles without pingpong:true (dead field)", () => {
    expectIssue(
      ...
      "loop_cycles has no meaning without pingpong:true",
    );
  });
  ...
  it("rejects loop_cycles that isn't a 2-element array", () => {
    expectIssue(
      ...
      "loop_cycles must be an array of 2 positive integers with lo<=hi",
    );
  });
...
describe("validateMotions — root_lock_y", () => {
  it("passes root_lock_y through", () => {
    const out = validateMotions(FILE, { idle: baseEntry({ root_lock_y: true }) });
    expect(out.idle.root_lock_y).toBe(true);
  });

  it("leaves root_lock_y absent when the entry omits it", () => {
    const out = validateMotions(FILE, { idle: baseEntry({}) });
    expect(out.idle.root_lock_y).toBeUndefined();
  });

  it("rejects a non-boolean root_lock_y", () => {
    expectIssue({ idle: baseEntry({ root_lock_y: "true" }) }, "root_lock_y must be a boolean");
  });
});
...
describe("validateMotions — crossfade_loop", () => {
  ...
  it("rejects a non-boolean crossfade_loop", () => {
    expectIssue(
      ...
      "crossfade_loop must be a boolean",
    );
  });
  ...
  it("rejects crossfade_loop:true without loop:true", () => {
    expectIssue(
      ...
      "crossfade_loop:true requires loop:true",
    );
  });
});
...
describe("validateMotions — fade_ms", () => {
  it("rejects a non-integer", () => {
    expectIssue({ idle: baseEntry({ fade_ms: 100.5 }) }, "fade_ms must be an integer in [0, 5000]");
  });

  it("rejects a negative value", () => {
    expectIssue({ idle: baseEntry({ fade_ms: -1 }) }, "fade_ms must be an integer in [0, 5000]");
  });

  it("rejects a value above 5000", () => {
    expectIssue({ idle: baseEntry({ fade_ms: 5001 }) }, "fade_ms must be an integer in [0, 5000]");
  });
