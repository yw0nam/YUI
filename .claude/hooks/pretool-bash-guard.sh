...
# Protect resources/purchased_motions from agent mutation (move/copy/delete/
# overwrite) and from being staged/pushed to git (resources/purchased_motions/
# AGENTS.md). Reads (cat/ls/grep) pass. YUI_ALLOW_MOTIONS=1 bypasses.
if [ "${YUI_ALLOW_MOTIONS:-}" != "1" ] \
  ...
  if printf '%s' "$cmd" | grep -qE '(^|[;&|`]|[[:space:]])(rm|mv|cp|rsync|install|ln|touch|truncate|shred|dd|mkdir|rmdir|chmod|chown|chflags)([[:space:]]|$)' \
    ...
    deny "resources/purchased_motions is protected — purchased motion files must not be moved, copied, deleted, overwritten, or staged/pushed to git by the agent (resources/purchased_motions/AGENTS.md). Reading is fine. If you are human-approved to curate these, re-run with YUI_ALLOW_MOTIONS=1."
  fi
fi
