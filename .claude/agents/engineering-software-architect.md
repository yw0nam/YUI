...
- The `generate_express` cue contract handed to the backend agent (`docs/backend_agent_broker_interaction.md`) — keep the tool args/streaming shape it describes aligned with `types.ts`.
...
- When the change affects what the backend agent emits, also align `docs/backend_agent_broker_interaction.md` (the cue contract handoff) and coordinate with the Technical Writer.
