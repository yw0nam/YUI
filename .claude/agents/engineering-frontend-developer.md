...
- Control signals arrive as server-side `generate_express` tool-calls in the stream, flat args `{ emotion_id?, motion_id?, emotion_text? }` — no `should_speak`. `function_call` items are excluded from final `output[]`, so they MUST be captured during streaming, not read back afterward.
...
## Anti-patterns
- No brain in the client — parse and forward; do not judge what to speak. Empty/absent speech text means silence (D-NO-SPEAK-GATE), render it as-is.
