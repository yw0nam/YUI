...
- Silence is expressed by the backend returning no/empty speech text — the dispatcher must not invent a speak/don't-speak gate.
...
- No speak/don't-speak gate — never suppress based on a client-side speak decision.
